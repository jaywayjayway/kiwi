...
class Firewall (object):
    ...
    def rule_for(self, address, service,index):
        '''Generate an iptables rule (returned as a tuple) for the given
        address and service.'''

        return iptables.Rule(str(arg) for arg in [
            '-d', address,
            '-p', service['spec']['ports'][index]['protocol'].lower(),
            '--dport', service['spec']['ports'][index]['port'],
            '-m', 'comment',
            '--comment', service['id'],
            '-j', 'MARK', '--set-mark', self.fwmark
        ])

    def add_service(self, address, service, index):
        '''Add a new service to the firewall.'''

        rule = self.rule_for(address, service, index)
        if rule in self.rules:
            LOG.info('not adding rule for service %s '
                     'on %s port %d (already exists)',
                     service['id'], address, service['spec']['ports'][index]['port'])
            return
    ...
    def remove_service(self, address, service, index):
        ...
        try:
            ...
            iptables.mangle.chains[self.fwchain].delete(rule=rule)
        except iptables.CommandError as exc:
            raise FirewallDriverError(reason=exc)
