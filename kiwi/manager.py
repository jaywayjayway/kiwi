...
class Manager (object):
    ...
    def handle_add_service(self, msg):
        service = msg['service']
        for address in service['spec'].get('externalIPs', []):
            ...
            if self.fw_driver:
                # index 为列表的索引, v 是值
                for index,v in enumerate(service["spec"]["ports"]):
                    try:
                        self.fw_driver.add_service(address, service, index)
                    except FirewallDriverError as exc:
                        LOG.error('failed to configure host firewall: %d',
                                  exc.returncode)

            try:
		self.addresses[address].setdefault('count',0)
	...
	LOG.info("service: %s", service)
        for address in service['spec'].get('externalIPs', []):
            ...
            if self.fw_driver:
                ...
                for index,v in enumerate(service["spec"]["ports"]):
                    try:
                        self.fw_driver.remove_service(address, service, index)
                    except FirewallDriverError as exc:
                        LOG.error('failed to configure host firewall: %d',
                                  exc.returncode)
